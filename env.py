...
class PredatorPreyEnv:
    ...
    def _get_predator_obs(self) -> np.ndarray:
        """
        Observation for each predator (dim 40) :

        [x, y, speed, heading]                   -> 4
        + 6 prey * (dx, dy, heading_j)        -> 18
        + 6 predators * (dx, dy, heading_j)    -> 18
    ...
    def _couzin_forces(self) -> np.ndarray:
        """
        Social forces (repulsion / alignment / attraction) for the prey.
        Returns an array (n_prey, 2) of "pseudo-accelerations".
        """
    ...
    def step(self, predator_actions):
        """
        predator_actions : np.array (n_predators, 2) -> acceleration vectors (ax, ay)
        """
        predator_actions = np.asarray(predator_actions)

        # --- Prey : Couzin forces + possible noise ---
        ...
        for i, pred in enumerate(self.pred_pos):
            ...
            rewards[i] += np.sum(dists < self.catch_radius)   # +1 per caught prey
            rewards[i] -= 0.01 * np.linalg.norm(predator_actions[i])  # energy cost

        # prey metrics
        dos = degree_of_sparsity(self.prey_pos)
        doa = degree_of_alignment(self.prey_vel)

        done = False  # no internal termination condition, cut off in train.py
